...
// Custom tooltip styled to match theme
function ChartTooltip({ active, payload, label }: any) {
  if (!active || !payload?.length) return null;
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
      {label && <p style={{ color: 'var(--text-muted)', marginBottom: 4 }}>{label}</p>}
      {payload.map((p: any) => (
        <p key={p.name} style={{ color: p.color || 'var(--text-primary)' }}>{p.name}: {p.value}</p>
      ))}
    </div>
  );
}
...
export default function AnalyticsPage() {
  ...
  // Chart 1: incidents per day (last 30 days)
  ...
  // Chart 2: incidents by hour of day
  ...
  const axisStyle = { fontFamily: 'var(--font-mono)', fontSize: 10, fill: 'var(--text-muted)' } as const;
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        {/* Row 1: Daily line + Hourly bar side by side */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 16 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignContent: 'start' }}>
            ...
            {[
              ...
              <div key={s.label} style={{ position: 'relative', overflow: 'hidden' }}>
